...
Заменяет ВСЕ вхождения в элементе, а не только первое
"""
...
def replace_all_in_text_nodes(element, words_to_replace):
    ...
    words_to_replace: set of words to replace
    """
    replaced_count = 0
    ...
    def process_node(node):
        ...
        if isinstance(node, NavigableString):
            ...
            # Для каждого слова, которое нужно заменить
            for word in words_to_replace:
                ...
                # Заменяем ВСЕ вхождения этого слова
            ...
            # Это элемент, рекурсивно обрабатываем его содержимое
            # Создаем копию списка, так как мы будем модифицировать дерево
...
def fix_file_with_soup(filepath, file_issues):
    ...
    # Группируем по element_id и собираем все уникальные слова для каждого элемента
    issues_by_element = {}
    ...
    for elem_id, words_to_replace in issues_by_element.items():
        ...
        # Заменяем все вхождения всех слов в этом элементе
...
def main():
    ...
    for filename, file_issues in sorted(files_to_fix.items()):
        ...
        if fixed_count > 0:
            ...
            print(f"⚠️  {filename}: 0 из {expected} мест")
